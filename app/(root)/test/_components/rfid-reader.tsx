...
const RFIDReader = ({ setTags }: { setTags: (tags: string[]) => void }) => {
    const [isReading, setIsReading] = useState(false);

    const extractValue = (input: string): string | null => {
    ...
    useEffect(() => {
        return () => {
            setIsReading(false); // Ensure reading is stopped on component unmount
        };
    }, []);

    return (
        <button onClick={isReading ? () => setIsReading(false) : startReading}>
            {isReading ? 'Stop Reading' : 'Start Reading RFID'}
        </button>
    );
};
